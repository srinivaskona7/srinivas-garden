...
const { Plant, Garden, createGarden } = require('../config/memoryDb');
...
router.get('/', async (req, res) => {
    try {
        ...
        const gardens = await (await Garden.find(query)).populate('plants').sort(sort).limit(parseInt(limit)).skip(0);
...
router.post('/:id/plants/:plantId', async (req, res) => {
    try {
        ...
        const updatedGarden = await Garden.findById(garden._id);
